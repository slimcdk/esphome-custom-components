...
void TMC5241::loop() { ESP_LOGD(TAG, "%.f", this->read_supply_voltage_mV()); }

void TMC5241::dump_config() {
...
uint32_t TMC5241::read_field(RegisterField field) {
  uint32_t value = read_register(field.address);
  return this->extract_field(value, field);
}

void TMC5241::write_field(RegisterField field, uint32_t value) {
...
uint32_t TMC5241::extract_field(uint32_t data, RegisterField field) {
...
float TMC5241::read_supply_voltage_mV() { return (float) this->read_field(TMC5240_ADC_VSUPPLY_FIELD) * 9.732; }
float TMC5241::read_adc_ain_mV() { return (float) this->read_field(TMC5240_ADC_AIN_FIELD) * 305.2; }
float TMC5241::read_temp_C() { return ((float) this->read_field(TMC5240_ADC_TEMP_FIELD) - 2039) / 7.7; }
int32_t TMC5241::get_vactual() { return CAST_Sn_TO_S32(this->read_register(TMC5240_VACTUAL), 23); }
