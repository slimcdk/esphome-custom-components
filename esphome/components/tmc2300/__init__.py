...
TMC2300API = tmc2300_ns.class_("TMC2300API", uart.UARTDevice)
TMC2300Component = tmc2300_ns.class_("TMC2300Component", TMC2300API, cg.Component)

DriverStatusEvent = tmc2300_ns.enum("DriverStatusEvent")

OnDriverStatusTrigger = tmc2300_ns.class_("OnDriverStatusTrigger", automation.Trigger)
OnStallTrigger = tmc2300_ns.class_("OnStallTrigger", automation.Trigger)
...
CurrentsAction = tmc2300_ns.class_("CurrentsAction", automation.Action)
StallGuardAction = tmc2300_ns.class_("StallGuardAction", automation.Action)
CoolConfAction = tmc2300_ns.class_("CoolConfAction", automation.Action)
ChopConfAction = tmc2300_ns.class_("ChopConfAction", automation.Action)
PWMConfAction = tmc2300_ns.class_("PWMConfAction", automation.Action)

DEVICE_SCHEMA = cv.Schema({cv.GenerateID(CONF_TMC2300_ID): cv.use_id(TMC2300Component)})
...
async def register_tmc2300_base(var, config):

    await cg.register_component(var, config)
    await uart.register_uart_device(var, config)

    enable_pin = config.get(CONF_ENABLE_PIN, None)
    ...
    if enable_pin is not None:
        ...
        cg.add(var.set_enable_pin(await cg.gpio_pin_expression(enable_pin)))

    if nstdby_pin is not None:
        cg.add_define("HAS_NSTDBY_PIN")
...
def validate_tmc2300_base(config):
    return config
...
@automation.register_action(
    "tmc2300.enable",
    ActivationAction,
    maybe_simple_id({cv.GenerateID(): cv.use_id(TMC2300Component)}),
)
...
@automation.register_action(
    ...
    maybe_simple_id(
        {
            cv.GenerateID(): cv.use_id(TMC2300Component),
...
def tmc2300_currents_to_code(config, action_id, template_arg, args):
    var = cg.new_Pvariable(action_id, template_arg)
    yield cg.register_parented(var, config[CONF_ID])
...
def tmc2300_stallguard_to_code(config, action_id, template_arg, args):
    var = cg.new_Pvariable(action_id, template_arg)
    yield cg.register_parented(var, config[CONF_ID])
...
def tmc2300_coolconf_to_code(config, action_id, template_arg, args):
    var = cg.new_Pvariable(action_id, template_arg)
    yield cg.register_parented(var, config[CONF_ID])
...
def tmc2300_chopconf_to_code(config, action_id, template_arg, args):
    var = cg.new_Pvariable(action_id, template_arg)
    yield cg.register_parented(var, config[CONF_ID])
...
def tmc2300_pwmconf_to_code(config, action_id, template_arg, args):
    var = cg.new_Pvariable(action_id, template_arg)
    yield cg.register_parented(var, config[CONF_ID])
...
TMC2300_FINAL_VALIDATE_SCHEMA = uart.final_validate_device_schema(
    CONF_TMC2300, require_rx=True, require_tx=True
)
