...
- [Actions](#actions)
  ...
  <!-- - [`tmc22xx.sync` Action](#tmc22xxsync-action) -->
- [Sensors](#sensors)
...
* `tmc22xx_hub_id` (**Required**, [ID][config-id]): Specify the ID of the hub this stepper is connected to. May be be left out if only a single or no `tmc22xx_hub` is configured
...
* `id` (**Required**, [ID][config-id]): Reference to the master tmc22xx.

* `to` (**Required**, list, [ID][config-id]): Reference to tmc22xx which settings should be applied to. -->
