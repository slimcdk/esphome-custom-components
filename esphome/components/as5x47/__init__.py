...
AS5X47ComponentConfigureAction = as5x47_ns.class_("AS5X47ComponentConfigureAction", automation.Action)
...
def tmc2209_configure_to_code(config, action_id, template_arg, args):
    ...
    if CONF_ABI_PULSE_PER_REVOLUTION in config:
        template_ = yield cg.templatable(config[CONF_ABI_PULSE_PER_REVOLUTION], args, int)
