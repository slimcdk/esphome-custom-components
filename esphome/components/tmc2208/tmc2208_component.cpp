...
void TMC2208Component::setup() {
  ESP_LOGCONFIG(TAG, "Setting up TMC2208 Component...");
  ...
  ESP_LOGCONFIG(TAG, "TMC2208 Component setup done.");
}
...
uint16_t TMC2208Component::current_scale_to_rms_current_mA(uint8_t cs) {
...
uint8_t TMC2208Component::rms_current_to_current_scale_mA_no_clamp(uint16_t mA) {
  return 32.0f * sqrtf(2) * FROM_MILLI(mA) * ((this->rsense_.value_or(0.170) + 0.02f) / this->read_vsense()) - 1.0f;
}

uint8_t TMC2208Component::rms_current_to_current_scale_mA(uint16_t mA) {
  if (mA == 0) {
    return 0;
  }
...
// void TMC2208Component::enable(bool enable, bool recover_toff = true) {
void TMC2208Component::enable(bool enable) {
  if (this->enn_pin_ != nullptr) {
